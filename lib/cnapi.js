...
const customerAuthKey = process.env.CUSTOMER_AUTH_KEY ?? "";

function apiInit(apiUrl, apiKey) {
  const cache = new InMemoryCache();

  console.log(`apiInit: apiUrl: ${apiUrl}`);
  console.log(`apiInit: apiKey: ${apiKey}`);
  console.log(`apiInit: customerAuthKey: ${customerAuthKey}`);
...
async function apiBulkNonprofits(query) {
  ...
  const sizes = getIncludedSizes(query.sizes);
  const alerts =
    typeof query.alerts === "string" ? [query.alerts] : query.alerts;
